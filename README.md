...
2. **문제 등록**: 두 가지 방법이 있습니다.
   - 직접 입력: 지침 내용을 보면서 OX/객관식/빈칸채우기 문제를 직접 입력합니다.
   - 자동 생성: `docs/guidelines/`에 넣어둔 지침 원문(PDF)을 텍스트로 변환해두면,
     규칙 기반으로 OX/빈칸채우기 문제 초안을 자동으로 뽑아줍니다. AI API 호출 없이
     순수 규칙(숫자·기간·정의 용어 탐지, 반의어 치환)으로 만들기 때문에 품질이
     들쭉날쭉할 수 있어요 — 생성된 문제를 검토해서 체크 해제/수정 후 추가하세요.
...
`docs/guidelines/*.pdf`를 추가하거나 바꾼 뒤 아래 명령을 실행하면
`public/guideline-texts/`에 정리된 텍스트(.md)와 `index.json` 목록이 생성/갱신됩니다.
이 폴더는 앱의 "지침에서 자동 생성" 기능이 읽는 곳이라, PDF를 추가했으면 꼭 다시
실행하고 커밋해야 합니다.
